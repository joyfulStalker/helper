<template>
	<view class="login">
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="form.requester" type="text" maxlength="30" placeholder="请使用账户或手机号" :focus="isFocus"></wInput>
				<wInput v-model="form.loginPassword" type="password" maxlength="11" placeholder="请输入密码"></wInput>
			</view>
	...
	export default {
		...
		methods: {
			...
			startLogin(e) {
				...
				if (this.$data.form.requester.length == "") {
					uni.showToast({
						icon: 'none',
						position: 'bottom',
						title: '用户名不能为空'
				...
				if (this.$data.form.loginPassword.length < 5) {
					uni.showToast({
						icon: 'none',
						position: 'bottom',
						title: '密码不正确'
				...
				// 	//console.log(res)
				// 	//简单验证下登录（不安全）
				// 	if(_this.requester==res.data.username && _this.loginPassword==res.data.password){
				// 		let userdata={
